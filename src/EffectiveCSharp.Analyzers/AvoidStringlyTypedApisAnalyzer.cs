...
public class AvoidStringlyTypedApisAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void AnalyzeLiteralExpression(SyntaxNodeAnalysisContext context)
    {
        ...
        {
            if (semanticModel.GetDeclaredSymbol(containingClass, context.CancellationToken) is INamedTypeSymbol containingTypeSymbol)
            {
        ...
        {
            if (semanticModel.GetDeclaredSymbol(containingMethod, context.CancellationToken) is IMethodSymbol methodSymbol)
            {
